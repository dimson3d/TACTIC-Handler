...
from lib.environment import env_mode, env_inst, env_server, dl, env_write_config, env_read_config, cfg_controls
...
class Ui_mainTabs(QtGui.QWidget, ui_main_tabs.Ui_mainTabsForm):
    def __init__(self, project_code, parent=None):
        ...
        dl.log('Init Main Tabs', group_id=project_code)
        ...
        dl.log('Creating Main Tabs', group_id=project_code)
...
class Ui_Main(QtGui.QMainWindow, ui_main.Ui_MainWindow):
    ...
    def menu_bar_actions(self):
        ...
        def close_routine():
            if env_mode.get_mode() == 'maya':
                from lib.ui_classes.ui_maya_dock import close_all_instances
                close_all_instances()
                self.close()
            if env_mode.get_mode() == 'standalone':
                self.close()
    ...
    def query_projects(self, result=None, run_thread=False):
        if result:
            ...
            #         self.restart_ui_main()
            # else:
            #     self.projects_items = result
                # self.save_object('projects_items', self.projects_items)
        ...
        if run_thread:
            ...
            if projects_cache:
                ...
                projects_items_worker.try_start()
                # if not self.projects_items_thread.isRunning():
